...
// Upstream's Hello World driver, with the two greeting senders replaced by FCE-B's
// single EXTRACT_SIGNAL sender. DeployInstructionSender and SetExtensionId are unchanged.
//
...
// SendExtractSignal submits one EXTRACT_SIGNAL instruction and returns its instruction id,
// which is the handle the proxy's result endpoint is polled with.
//
// message is the JSON handler.Request: {"callId","source":{...},"post":{...}} — it carries
// FCE-A's signed attestation and the post plaintext, because FCE-B refuses to sign unless
// the text reproduces the content hash FCE-A committed to.
//
// Expect to call this TWICE per extraction: the first instruction primes the model call and
// answers deferred, a later one collects it. See pkg/handler/deferred.go.
func SendExtractSignal(s *support.Support, instructionSenderAddress common.Address, message []byte) (common.Hash, common.Hash, error) {
	sender, err := helloworld.NewHelloWorldInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	tx, err := sender.SendExtractSignal(opts, message)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := helloworld.HelloWorldInstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("sendExtractSignal", message)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
