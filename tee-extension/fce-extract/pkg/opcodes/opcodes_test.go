...
func TestSolidityConstantsMatch(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"OP_TYPE_KASSETTE_EXTRACT", OPTypeExtract, "OPTypeExtract"},
		{"OP_COMMAND_EXTRACT_SIGNAL", OPCommandExtract, "OPCommandExtract"},
	} {
		...
		t.Logf("%-26s %q -> 0x%s", tc.solName, got, hex.EncodeToString(h[:]))
	}
	...
	// ⭐ FCE-B must not reuse FCE-A's identifiers. If it did, one InstructionSender's
	// instructions would be answerable by the other extension's machines, and the
	// separation the two enclaves exist for would be gone from the wire while looking
	// intact in the contracts.
	if regexp.MustCompile(`bytes32\("KASSETTE_SOURCE"\)|bytes32\("FETCH_POST"\)`).Match(src) {
		t.Error("FCE-B's InstructionSender carries FCE-A's op type or command")
	}
}
...
func TestNamesAvoidReservedWords(t *testing.T) {
	...
	for _, name := range []string{OPTypeExtract, OPCommandExtract} {
		if name == "" {
...
// The two extensions must be distinguishable on the wire.
func TestNamesDifferFromFceA(t *testing.T) {
	if OPTypeExtract == "KASSETTE_SOURCE" {
		t.Error("FCE-B reuses FCE-A's op type")
	}
	if OPCommandExtract == "FETCH_POST" {
		t.Error("FCE-B reuses FCE-A's op command")
	}
}
