...
export function saveMark(db: DatabaseSync, callId: number, kind: MarkKind, m: Mark, at: number) {
  db.prepare(
...
export async function markCall(
  callId: number,
  opts: { db?: DatabaseSync; status?: FspStatus; now?: number } = {}
): Promise<MarkResult> {
  ...
  if (!row.feed_id) {
    db.prepare("UPDATE calls SET status = 'unpriceable' WHERE id = ?").run(callId);
  ...
  const has = (kind: MarkKind) => !!db.prepare("SELECT 1 FROM marks WHERE call_id = ? AND kind = ?").get(callId, kind);
  ...
  try {
    const entry = has("entry") ? null : await priceAt(row.feed_id, row.posted_at, status);
    if (entry) saveMark(db, callId, "entry", entry, row.posted_at);

    const latest = await priceAt(row.feed_id, now, status);
    saveMark(db, callId, "latest", latest, now);
    ...
    if (!has("bench_entry")) {
      ...
      if (benchEntry) saveMark(db, callId, "bench_entry", benchEntry, row.posted_at);
    }
    saveMark(db, callId, "bench_latest", isOwnBenchmark ? latest : await priceAt(XRP_USD, now, status), now);
  } catch (e) {
...
export async function markOpenCalls(db?: DatabaseSync): Promise<MarkResult[]> {
